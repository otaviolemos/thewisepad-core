...
describe('Authentication middleware', () => {
  test('should return forbidden if access token is empty', async () => {
    ...
    expect(response).toEqual(forbidden(new MissingParamError('accessToken')))
  })

  test('should return forbidden if access token is null', async () => {
    const authMiddleware = new Authentication(new FakeTokenManager())
    const response: HttpResponse = await authMiddleware.handle({ accessToken: null })
    expect(response).toEqual(forbidden(new MissingParamError('accessToken')))
  })

  test('should return forbidden if access token is invalid', async () => {
